...
static gboolean uint64_equal(gconstpointer v1, gconstpointer v2)
{
    return *((const uint64_t *)v1) == *((const uint64_t *)v2);
}

static guint uint64_hash(gconstpointer v)
{
    return (guint)*(const uint64_t *)v;
}

static gboolean hash_remove_by_domain(gpointer key, gpointer value,
                                      gpointer user_data)
{
...
static gboolean hash_remove_by_page(gpointer key, gpointer value,
                                    gpointer user_data)
{
    VTDIOTLBEntry *entry = (VTDIOTLBEntry *)value;
 ...
 */
static void reset_context_cache(IntelIOMMUState *s)
{
 ...
 */
static void iommu_translate(VTDAddressSpace *vtd_as, int bus_num, int devfn,
...
static inline void vtd_context_global_invalidate(IntelIOMMUState *s)
{
    s->context_cache_gen++;
 ...
 */
static inline void vtd_context_device_invalidate(IntelIOMMUState *s,
                                                 uint16_t source_id,
                                                 uint16_t func_mask)
{
    uint16_t mask;
...
static uint64_t vtd_context_cache_invalidate(IntelIOMMUState *s, uint64_t val)
{
    ...
    default:
        VTD_DPRINTF(GENERAL,
                    "error: invalid context-cache invalidation granularity, "
                    "use global invalidation instead");
        caig = VTD_CCMD_GLOBAL_INVL_A;
        vtd_context_global_invalidate(s);
    }
...
{
    g_hash_table_foreach_remove(s->iotlb, hash_remove_by_domain, &domain_id);
}
                                      ...
                                      hwaddr addr, uint8_t am)
{
    ...
    info.mask = ~((1 << am) - 1);
    g_hash_table_foreach_remove(s->iotlb, hash_remove_by_page, &info);
}
                                         ...
                                         bool is_write)
{
    ...
    iommu_translate(vtd_as, vtd_as->bus_num, vtd_as->devfn, addr,
                    is_write, &ret);

    VTD_DPRINTF(MMU,
                "bus %d slot %d func %d devfn %d gpa %"PRIx64 " hpa %"PRIx64,
                vtd_as->bus_num, VTD_PCI_SLOT(vtd_as->devfn),
                VTD_PCI_FUNC(vtd_as->devfn), vtd_as->devfn,
                addr, ret.translated_addr);
    return ret;
}
...
static void vtd_realize(DeviceState *dev, Error **errp)
{
    ...
    s->iotlb = g_hash_table_new_full(uint64_hash, uint64_equal, g_free, g_free);
    do_vtd_init(s);
}
